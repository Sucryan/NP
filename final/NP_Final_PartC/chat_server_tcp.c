        ...
        for (each_socket = 1; each_socket <= max_socket; ++each_socket) {
            ...
            if (FD_ISSET(each_socket, &read_ready)) {
                if(each_socket == socket_listen) {
                    ...
                    SSL* secure_socket = SSL_new(ssl_context);
                    if (!secure_socket) {
                    ...
                    // 這邊額外補一段SSL_set_fd，是因為要先確認TLS有成功，然後再正式把它塞進active_socket裡面
                    SSL_set_fd(secure_socket, socket_client);

                    int SSL_accept_result = SSL_accept(secure_socket);
                    ...
                    // 看到socket_client: (fd)，知道要對應的用secure_socket來解碼。
                    ssl_clients[socket_client] = secure_socket;
                    ...
                    if(bytes_received < 1) {
                        ...
                        ssl_clients[socket_client] = NULL;
                        continue;
                    }
                    // 不接會髒髒的
                    if(bytes_received > 0) {
                    ...
                    // 送回去給client
                    SSL_write(secure_socket, greeting_msg, strlen(greeting_msg));
                    ...
                    if(bytes_received < 1) {
                        ...
                        SSL_shutdown(ssl_clients[each_socket]);
                        CLOSESOCKET(each_socket);
                        SSL_free(ssl_clients[each_socket]);
                        ssl_clients[each_socket] = NULL;
                    ...
                    // 額外插入讀指令的部分，讀到開頭是/代表就是指令了，後面可以直接接continue
                    if (received[0] == '/') {
                        if (strcmp(received, "/who") == 0) {
                            char response[4096];
                            snprintf(response, sizeof(response), "Online users:\n");
                            // 跑一整段active_sockets, 抓出所有使用者的名字
                            for (int i = 1; i <= max_socket; i++) {
                                if (
                                    FD_ISSET(i, &active_sockets) &&
                                    i != socket_listen &&
                                    names[i][0] != '\0'
                                ) {
                                    snprintf(
                        ...
                        SSL_write(
                            ssl_clients[each_socket],
                            "Unknown command. Try /who\n",
                            strlen("Unknown command. Try /who\n")
                        );
                    ...
                    for(int i = 1; i <= max_socket; i++) {
                        if(FD_ISSET(i, &active_sockets)) {
                            ...
                            if(i != socket_listen && i != each_socket) {
                                int send_result = SSL_write(ssl_clients[i], to_send, strlen(to_send));
                            }
                        }
                    }
                }
